...
function L0Init(args)
    ...
    if(#args ~= 2)
    then
        print("ERR :" .. CName ..  " L0Init --- wrong args number", #args)
        return false
    end
...
-- 每次合约执行都调用
-- 发起提现 launch & 撤销提现 cancel & 提现成功 succeed & 提现失败 failed
...
function launch(args) 
    ...
    if(#args ~= 1)
    then
        print("ERR :" .. CName ..  " launch --- wrong args number", #args)
        return false
    end
...
function cancel(args)
    ...
    if(#args ~= 1)
    then
        print("ERR :" .. CName ..  " cancel --- wrong args number", #args)
        return false
    end
    -- execute
    local withdrawID = "launch_"..args[0]
    ...
    if (not withdrawInfo) 
    then
        print("ERR :" .. CName ..  " cancel --- id not exist")
        return false
    end
    local txInfo = L0.TxInfo()
    local sender = txInfo["Sender"]
    ...
    if (receiver ~= sender) 
    then
        print("ERR :" .. CName ..  " cancel --- wrong sender")
        return false
    end
    -- to do balance check
...
function succeed(args)
    ...
    if(#args ~= 2)
    then
        print("ERR :" .. "succeed --- wrong args number", #args)
        return false
    end
    -- execute
    local withdrawID = "launch_"..args[0]
...
function fail(args)
    ...
    if (system ~= sender) 
    then
        print("ERR :" .. CName ..  " fail --- wrong sender, must system")
        return false
    end
    ...
    if (not withdrawInfo) 
    then
        print("ERR :" .. CName ..  " fail --- withdrawID not exist")
        return false
    end
    local tb = string.split(withdrawInfo, "&")
    receiver = tb[1]
